...
function resetAgentPlan(){ state.lastAgentPlanId=""; state.lastAgentPlan=null; const field=$("agentCandidateField"), recommendation=$("agentPlanRecommendation"), view=$("agentPlanView"); if(field) field.classList.add("hidden"); if(recommendation) recommendation.classList.add("hidden"); if(view) view.value=""; }
...
function updateAgentPlan(plan,rendered=""){ plan.rendered=rendered||plan.rendered||""; state.lastAgentPlanId=plan.plan_id; renderAgentCandidates(plan); if($("agentPlanDialog")?.open) renderAgentPlanDialog(); }
...
async function confirmAgentPlanGeneration(){ requireBook(); if(!state.lastAgentPlanId) throw new Error("请先生成并确认章节规划"); const candidate=selectedAgentCandidate(); const data=await api(`/api/books/${enc(state.currentBook)}/agent/chapter/generate`,{method:"POST",body:JSON.stringify({plan_id:state.lastAgentPlanId,candidate_id:(candidate&&candidate.candidate_id)||$("agentCandidateSelect")?.value||""})}); closeAgentPlanDialog(); connectTask(data.task_id); selectSection("tasks"); }
...
function openTokenContent(index){ const row=(state.tokenEntries||[])[index]; const dialog=$("tokenContentDialog"); if(!dialog||!row) return; const full=(row.content_full||"").trim(); $("tokenContentFull").textContent=full||(row.content_preview||"(无内容)"); $("tokenContentMeta").textContent=[row.timestamp,row.direction==="send"?"发送":"接收",row.operation,row.strategy,row.model].filter(Boolean).join(" · "); $("tokenContentNote").classList.toggle("hidden",Boolean(full)); if(!dialog.open) dialog.showModal(); }
