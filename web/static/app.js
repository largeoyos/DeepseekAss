const state = {
  ...
  lastAgentPlanId: "",
  lastAgentPlan: null,
  lastGeneratedAgentPlanId: "",
  lastAgentGenerationVersion: 0,
  lastPolishPlanId: "",
  lastExtraPlanId: "",
...
function setCurrentBook(title){ const nextTitle=title||""; if(state.currentBook && state.currentBook!==nextTitle) resetAgentPlan(); state.currentBook=nextTitle; if(title) sessionStorage.setItem("deepseekass_book",title); else sessionStorage.removeItem("deepseekass_book"); $("currentBookTitle").textContent=title||"-"; const contSelect=$("contBookSelect"); if(contSelect) contSelect.value=title||""; const contTitle=$("contAnalysisTitle"); if(title && contTitle && !contTitle.value.trim()) contTitle.value=title; }
...
async function resumeAgentAfterChange(approved,changeSetId,runId=""){ if(!state.activeAgentRunId || (runId&&runId!==state.activeAgentRunId)) return; try{ await controlAgentRun("resume",{approved,change_set_id:changeSetId}); }catch(e){ if(!String(e.message||"").includes("活动状态")) throw e; } }
...
function renderPendingChangeList(box, changes){ if(!box) return; renderCards(box, changes||[], cs=>{ const id=cs.change_set_id||""; const ops=changePatchOperations(cs); const source=((cs.review||{}).origin_label)||(((cs.validation_result||{}).origin)==="external_control"?"外部 AI":"内置 Agent"); const b=buttonCard(id||"变更", `${source} · ${cs.status||"pending"} · ${ops.length||(cs.operations||[]).length} 项 · ${cs.reason||""}`, id===state.selectedChangeSetId?"已选":"选择", id===state.selectedChangeSetId); b.onclick=()=>selectPendingChange(cs); return b; }); }
...
function selectPendingChange(cs){ state.selectedChangeSetId=cs.change_set_id||""; const ops=changePatchOperations(cs); const review=cs.review||{}; const lines=[`ChangeSet: ${state.selectedChangeSetId}`, `来源: ${review.origin_label||"内置 Agent"}`, `状态: ${cs.status||""}`, `原因: ${cs.reason||""}`, ""]; if(ops.length){ lines.push("拟议世界书变更:"); ops.forEach((op,i)=>lines.push(`${i+1}. ${op.operation} ${op.entity_type}:${op.entity_id}\n   scope=${op.scope||"uncertain"} anchor=${op.anchor_node_id||""}\n   reason=${op.reason||""}\n   payload=${JSON.stringify(op.payload||{})}`)); } else { lines.push("拟议章节/项目变更:"); (review.operations||[]).forEach((op,i)=>{ lines.push(`${i+1}. ${op.operation} ${op.chapter_title||op.target_id||""}`); if(op.base_node_id) lines.push(`   基准节点=${op.base_node_id}`); if(op.kind==="chapter") lines.push(`   批准后激活=${op.will_activate?"是":"否"}，字符数 ${op.before_chars||0} -> ${op.after_chars||0}\n${op.diff||"无文本差异"}`); else lines.push(JSON.stringify(op.payload||op)); }); } previewAgentText(ops.length?"世界书变更审批":"章节变更审批",lines.join("\n")); renderChangeScopeEditor(ops); renderPendingChanges(state.pendingChanges); }
...
async function approveSelectedChange(){ requireBook(); const cs=selectedChangeSet(); if(!cs) throw new Error("请选择一组待审批变更"); const operations=changePatchOperations(cs); if(operations.length){ const confirmed=collectScopeOperations(); await api(`/api/books/${enc(state.currentBook)}/agent/world/confirm-scopes`,{method:"POST",body:JSON.stringify({change_set_id:cs.change_set_id,operations:confirmed})}); } await api(`/api/books/${enc(state.currentBook)}/agent/changes/approve`,{method:"POST",body:JSON.stringify({change_set_id:cs.change_set_id})}); await resumeAgentAfterChange(true,cs.change_set_id,cs.run_id||""); state.selectedChangeSetId=""; renderChangeScopeEditor([]); toast("变更已批准并已创建快照"); await loadAgentState(false); if(operations.length) await loadWorld().catch(()=>{}); }
...
async function rejectChange(id){ const cs=(state.pendingChanges||[]).find(item=>item.change_set_id===id)||{}; await api(`/api/books/${enc(state.currentBook)}/agent/changes/reject`,{method:"POST",body:JSON.stringify({change_set_id:id})}); await resumeAgentAfterChange(false,id,cs.run_id||""); toast("变更已拒绝"); await loadAgentState(false); }
...
function agentPlanCandidates(plan){ const candidates=Array.isArray(plan&&plan.candidate_plans)?plan.candidate_plans.filter(Boolean):[]; return candidates.length?candidates:[plan].filter(Boolean); }
function agentCandidateId(candidate,index=0){ return (candidate&&candidate.candidate_id)||(candidate&&candidate.plan_id)||String(index); }
function agentCandidateScore(candidate){ const critic=(candidate&&candidate.critic)||{}; const keys=["causality","character_agency","surprise","main_plot_value"]; const scores=keys.map(key=>Number(critic[key])).filter(Number.isFinite); return scores.length?` · Critic ${scores.reduce((sum,value)=>sum+value,0)}/${scores.length*10}`:""; }
function selectedAgentCandidate(){ const plan=state.lastAgentPlan; const dialog=$("agentPlanDialog"); const selected=(dialog&&dialog.open?$("agentPlanDialogCandidateSelect")?.value:$("agentCandidateSelect")?.value)||""; return agentPlanCandidates(plan).find((item,index)=>agentCandidateId(item,index)===selected)||plan||null; }
function agentPlanSourceText(candidate){ const report=(candidate&&candidate.context_report)||{}; const lines=[`候选上下文：${report.candidate_chars||0} 字`,`实际注入：${report.injected_chars||0} 字`,`省略：${report.omitted_chars||0} 字`,""]; for(const skill of report.skills||[]){ lines.push(`- Skill ${skill.name||skill.id||""} v${skill.version||"1"}；来源=${skill.scope||""}；原因=${skill.reason||""}`); } if((report.skills||[]).length) lines.push(""); for(const item of report.sources||[]){ lines.push(`- ${item.title||""}: ${(item.content||"").length} 字；来源=${item.source||""}；原因=${item.reason||""}；省略=${item.omitted_chars||0}`); } return lines.join("\n"); }
function agentPlanDialogText(candidate,tab){ if(tab==="context") return ((candidate||{}).context_report||{}).content||"未记录实际注入上下文。"; if(tab==="sources") return agentPlanSourceText(candidate); return (candidate&&candidate.rendered)||JSON.stringify(candidate||state.lastAgentPlan||{},null,2); }
function setAgentCandidateSelection(id){ const main=$("agentCandidateSelect"), dialog=$("agentPlanDialogCandidateSelect"); if(main) main.value=id; if(dialog) dialog.value=id; const candidate=selectedAgentCandidate(); if($("agentPlanView")) $("agentPlanView").value=(candidate&&candidate.rendered)||JSON.stringify(candidate||state.lastAgentPlan||{},null,2); renderAgentPlanDialogContent(); }
function fillAgentCandidateSelect(select,candidates,preferred){ if(!select) return; select.replaceChildren(); candidates.forEach((candidate,index)=>{ const id=agentCandidateId(candidate,index); const option=document.createElement("option"); option.value=id; option.textContent=`${index+1}. ${candidate.strategy||candidate.title||`方案 ${index+1}`}${agentCandidateScore(candidate)}${id===(state.lastAgentPlan?.recommended_candidate_id||"")?"（推荐）":""}`; select.append(option); }); select.value=preferred||agentCandidateId(candidates[0],0); }
function renderAgentCandidates(plan){ state.lastAgentPlan=plan; const candidates=agentPlanCandidates(plan); const field=$("agentCandidateField"), select=$("agentCandidateSelect"), recommendation=$("agentPlanRecommendation"); if(!field||!select||!recommendation) return; const preferred=(candidates.find((item,index)=>agentCandidateId(item,index)===(plan.recommended_candidate_id||""))||candidates[0]); const selected=agentCandidateId(preferred,0); fillAgentCandidateSelect(select,candidates,selected); field.classList.toggle("hidden",candidates.length<2); recommendation.textContent=candidates.length>1?`需要确认候选规划。当前推荐：${select.selectedOptions[0]?.textContent||"方案"}`:"当前规划已就绪，确认后生成正文。"; recommendation.classList.remove("hidden"); select.onchange=()=>setAgentCandidateSelection(select.value); setAgentCandidateSelection(select.value); }
function renderAgentPlanDialogContent(){ const dialog=$("agentPlanDialog"); if(!dialog||!dialog.open) return; const candidate=selectedAgentCandidate(); const tab=state.agentPlanDialogTab||"plan"; $("agentPlanDialogContent").textContent=agentPlanDialogText(candidate,tab); document.querySelectorAll("[data-agent-plan-tab]").forEach(button=>button.classList.toggle("active",button.dataset.agentPlanTab===tab)); }
function renderAgentPlanDialog(){ const dialog=$("agentPlanDialog"), plan=state.lastAgentPlan; if(!dialog||!plan) return; const candidates=agentPlanCandidates(plan); const select=$("agentPlanDialogCandidateSelect"), field=$("agentPlanDialogCandidateField"), recommendation=$("agentPlanDialogRecommendation"); const previous=select.value||$("agentCandidateSelect")?.value||""; fillAgentCandidateSelect(select,candidates,previous); field.classList.toggle("hidden",candidates.length<2); const current=selectedAgentCandidate()||candidates[0]; const critic=(current&&current.critic)||{}; const score=[['causality','因果'],['character_agency','主动性'],['surprise','意外性'],['main_plot_value','主线价值']].filter(([key])=>critic[key]!==undefined).map(([key,label])=>`${label} ${critic[key]}/10`).join('；'); const reason=(current&&current.selection_reason)||critic.reason||'当前方案可继续修改后确认。'; recommendation.textContent=`Critic 评估：${score||'当前方案已人工修改或未评分'}\n说明：${reason}${critic.risk?`\n风险提示：${critic.risk}`:""}`; select.onchange=()=>setAgentCandidateSelection(select.value); document.querySelectorAll("[data-agent-plan-tab]").forEach(button=>button.onclick=()=>{ state.agentPlanDialogTab=button.dataset.agentPlanTab; renderAgentPlanDialogContent(); }); renderAgentPlanDialogContent(); }
function openAgentPlanDialog(){ if(!state.lastAgentPlan) throw new Error("请先生成章节规划"); const dialog=$("agentPlanDialog"); if(!dialog) return; state.agentPlanDialogTab="plan"; if(!dialog.open) dialog.showModal(); renderAgentPlanDialog(); }
function closeAgentPlanDialog(){ const dialog=$("agentPlanDialog"); if(dialog&&dialog.open) dialog.close(); }
function refreshAgentRegenerateButton(){ const button=$("agentRegenerateBtn"); if(button) button.disabled=!state.lastAgentPlanId || state.lastGeneratedAgentPlanId!==state.lastAgentPlanId; }
function resetAgentPlan(){ state.lastAgentPlanId=""; state.lastAgentPlan=null; state.lastGeneratedAgentPlanId=""; state.lastAgentGenerationVersion=0; const field=$("agentCandidateField"), recommendation=$("agentPlanRecommendation"), view=$("agentPlanView"); if(field) field.classList.add("hidden"); if(recommendation) recommendation.classList.add("hidden"); if(view) view.value=""; refreshAgentRegenerateButton(); }
async function cancelAgentPlan(){ const planId=state.lastAgentPlanId; if(!planId){ closeAgentPlanDialog(); return; } await api(`/api/books/${enc(state.currentBook)}/agent/chapter/cancel`,{method:"POST",body:JSON.stringify({plan_id:planId})}); resetAgentPlan(); closeAgentPlanDialog(); toast("章节计划已取消"); }
function updateAgentPlan(plan,rendered=""){ plan.rendered=rendered||plan.rendered||""; if(state.lastAgentPlanId!==plan.plan_id){ state.lastGeneratedAgentPlanId=""; state.lastAgentGenerationVersion=0; } state.lastAgentPlanId=plan.plan_id; renderAgentCandidates(plan); refreshAgentRegenerateButton(); if($("agentPlanDialog")?.open) renderAgentPlanDialog(); }
async function agentPlan(){ requireBook(); const data=await api(`/api/books/${enc(state.currentBook)}/agent/chapter/plan`,{method:"POST",body:JSON.stringify({chapter_title:$("agentChapterTitle").value,plot:$("agentPlot").value,requirement:$("agentRequirement").value,target_words:Number($("agentWords").value||3000),manual_entity_ids:splitNames($("agentManualEntities").value)})}); connectTask(data.task_id,payload=>{ const result=(payload.data||{}).result; const plan=(payload.data||{}).plan || (result||{}).plan; if(plan){ updateAgentPlan(plan,(payload.data||{}).rendered || (result||{}).rendered || ""); openAgentPlanDialog(); } }); selectSection("tasks"); }
async function reviseAgentPlan(){ requireBook(); const instruction=$("agentPlanRevisionInput").value.trim(); if(!instruction) throw new Error("请填写具体的章节计划修改要求"); const candidate=selectedAgentCandidate(); const button=$("reviseAgentPlanBtn"), status=$("agentPlanDialogStatus"); button.disabled=true; status.textContent="AI 正在基于当前计划做局部修改，请稍候……"; try{ const data=await api(`/api/books/${enc(state.currentBook)}/agent/chapter/revise`,{method:"POST",body:JSON.stringify({plan_id:state.lastAgentPlanId,candidate_id:(candidate&&candidate.candidate_id)||"",instruction})}); connectTask(data.task_id,payload=>{ const result=(payload.data||{}).result; const plan=(payload.data||{}).plan || (result||{}).plan; if(plan){ updateAgentPlan(plan,(payload.data||{}).rendered || (result||{}).rendered || ""); $("agentPlanRevisionInput").value=""; status.textContent="当前章节计划已按要求修改，可继续修改或确认生成正文。"; toast("章节计划已修改"); } }); }finally{ button.disabled=false; } }
function agentGenerate(){ requireBook(); if(!state.lastAgentPlanId) throw new Error("请先生成并确认章节规划"); openAgentPlanDialog(); }
function markAgentPlanGenerated(planId,result,regenerated=false){ if(!result || !result.version) return; const isNewVersion=Number(result.version)!==Number(state.lastAgentGenerationVersion); state.lastGeneratedAgentPlanId=planId; state.lastAgentGenerationVersion=Number(result.version); refreshAgentRegenerateButton(); if(isNewVersion) toast(regenerated?`已按当前草案生成新版本 v${result.version}`:`章节已生成 v${result.version}，当前草案可直接重用`); }
async function confirmAgentPlanGeneration(){ requireBook(); if(!state.lastAgentPlanId) throw new Error("请先生成并确认章节规划"); const planId=state.lastAgentPlanId; const candidate=selectedAgentCandidate(); const data=await api(`/api/books/${enc(state.currentBook)}/agent/chapter/generate`,{method:"POST",body:JSON.stringify({plan_id:planId,candidate_id:(candidate&&candidate.candidate_id)||$("agentCandidateSelect")?.value||"",reuse_current_plan:false})}); closeAgentPlanDialog(); connectTask(data.task_id,payload=>{ const result=(payload.data||{}).result; markAgentPlanGenerated(planId,result,false); }); selectSection("tasks"); }
async function regenerateCurrentAgentPlan(){ requireBook(); if(!state.lastAgentPlanId || state.lastGeneratedAgentPlanId!==state.lastAgentPlanId) throw new Error("请先按当前草案完成一次章节生成"); if(!confirm("跳过 Agent 规划，直接按当前草案为同一章生成一个新版本？现有版本不会被覆盖。")) return; const planId=state.lastAgentPlanId; const button=$("agentRegenerateBtn"); button.disabled=true; try{ const data=await api(`/api/books/${enc(state.currentBook)}/agent/chapter/generate`,{method:"POST",body:JSON.stringify({plan_id:planId,candidate_id:"",reuse_current_plan:true})}); connectTask(data.task_id,payload=>{ const result=(payload.data||{}).result; markAgentPlanGenerated(planId,result,true); if(["failed","cancelled"].includes(payload.type)) refreshAgentRegenerateButton(); }); selectSection("tasks"); }catch(error){ refreshAgentRegenerateButton(); throw error; } }
...
const MODEL_STAGES=[
  ["interactive","互动对话"],["drafting","正文生成"],["planning","规划分析"],
  ["extraction","摘要提取"],["review","审校润色"],["agent","Agent 工具"]
];
function newProfileId(prefix){ return `${prefix}_${(crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random())).replaceAll("-","").slice(0,12)}`; }
function modelProfiles(){ return Object.values((state.modelConfig||{}).models||{}); }
function providerProfiles(){ return Object.values((state.modelConfig||{}).providers||{}); }
function setModelCenterSelects(){
  const providers=providerProfiles(), models=modelProfiles();
  if(!providers.some(item=>item.provider_id===state.selectedProviderId)) state.selectedProviderId=providers[0]?.provider_id||"";
  if(!models.some(item=>item.model_id===state.selectedModelProfileId)) state.selectedModelProfileId=models[0]?.model_id||"";
  $("providerProfileSelect").innerHTML=providers.map(item=>`<option value="${escapeHtml(item.provider_id)}">${escapeHtml(item.name)} · ${escapeHtml(item.protocol)}</option>`).join("");
  $("providerProfileSelect").value=state.selectedProviderId;
  $("modelProfileSelect").innerHTML=models.map(item=>`<option value="${escapeHtml(item.model_id)}">${escapeHtml(item.name)} · ${escapeHtml(item.model)}</option>`).join("");
  $("modelProfileSelect").value=state.selectedModelProfileId;
  $("modelProviderSelect").innerHTML=providers.map(item=>`<option value="${escapeHtml(item.provider_id)}">${escapeHtml(item.name)}</option>`).join("");
}
function fillProviderEditor(){
  const p=((state.modelConfig||{}).providers||{})[state.selectedProviderId]||{};
  $("providerName").value=p.name||""; $("providerProtocol").value=p.protocol||"openai_chat"; $("providerUrl").value=p.base_url||"";
  $("providerApiKey").value=p.api_key||""; $("providerAuthHeader").value=p.auth_header||"Authorization"; $("providerAuthPrefix").value=p.auth_prefix??"Bearer "; $("providerHeaders").value=JSON.stringify(p.headers||{},null,2); $("providerTimeout").value=p.timeout_seconds||180; $("providerRetries").value=p.max_retries??1;
  $("providerEnabled").checked=p.enabled!==false; $("providerInsecure").checked=!!p.allow_insecure_http;
}
function storeProviderEditor(){
  const p=((state.modelConfig||{}).providers||{})[state.selectedProviderId]; if(!p) return;
  let headers={}; try{ headers=JSON.parse($("providerHeaders").value.trim()||"{}"); }catch{ throw new Error("自定义 Headers JSON 无效"); }
  Object.assign(p,{name:$("providerName").value.trim()||p.name,protocol:$("providerProtocol").value,base_url:$("providerUrl").value.trim().replace(/\/$/,""),api_key:$("providerApiKey").value.trim(),auth_header:$("providerAuthHeader").value.trim()||"Authorization",auth_prefix:$("providerAuthPrefix").value,headers,timeout_seconds:Number($("providerTimeout").value||180),max_retries:Number($("providerRetries").value||0),enabled:$("providerEnabled").checked,allow_insecure_http:$("providerInsecure").checked});
}
function fillModelEditor(){
  const m=((state.modelConfig||{}).models||{})[state.selectedModelProfileId]||{}, c=m.capabilities||{}, d=m.defaults||{};
  $("modelProviderSelect").value=m.provider_id||""; $("modelProfileName").value=m.name||""; $("modelRealName").value=m.model||"";
  $("modelTemplate").value=m.template||"generic";
  $("modelContextWindow").value=m.context_window||32768; $("modelMaxOutput").value=m.max_output_tokens||32768; $("modelRuntimeContext").value=m.runtime_context||0;
  $("modelCapStream").checked=c.streaming!==false; $("modelCapTools").checked=!!c.tools; $("modelCapSearch").checked=!!c.native_web_search; $("modelCapReasoning").checked=!!c.reasoning; $("modelCapJson").checked=c.structured_output!==false;
  const ps=m.parameter_support||{}; $("modelParamTemperature").checked=ps.temperature!==false; $("modelParamTopP").checked=ps.top_p!==false; $("modelParamFrequency").checked=ps.frequency_penalty!==false; $("modelParamMaxTokens").checked=ps.max_tokens!==false;
  $("modelReasoningLevels").value=(c.supported_reasoning_levels||["off"]).join(", "); $("modelDefaultReasoning").value=d.reasoning_level||"off"; $("modelDefaultSearch").value=d.web_search||"off";
  $("modelDefaultTemperature").value=d.temperature??""; $("modelDefaultTopP").value=d.top_p??""; $("modelDefaultFrequency").value=d.frequency_penalty??"";
  $("modelExtraBody").value=JSON.stringify(m.extra_body||{},null,2);
  $("modelProfileEnabled").checked=m.enabled!==false;
}
function storeModelEditor(){
  const m=((state.modelConfig||{}).models||{})[state.selectedModelProfileId]; if(!m) return;
  let extra={}; try{ extra=JSON.parse($("modelExtraBody").value.trim()||"{}"); }catch{ throw new Error("高级请求 JSON 无效"); }
  m.provider_id=$("modelProviderSelect").value; m.name=$("modelProfileName").value.trim()||m.name; m.model=$("modelRealName").value.trim()||m.model;
  m.template=$("modelTemplate").value;
  m.enabled=$("modelProfileEnabled").checked;
  m.context_window=Number($("modelContextWindow").value||32768); m.max_output_tokens=Number($("modelMaxOutput").value||32768); m.runtime_context=Number($("modelRuntimeContext").value||0)||null; m.extra_body=extra;
  m.capabilities={...(m.capabilities||{}),streaming:$("modelCapStream").checked,tools:$("modelCapTools").checked,native_web_search:$("modelCapSearch").checked,reasoning:$("modelCapReasoning").checked,structured_output:$("modelCapJson").checked,supported_reasoning_levels:$("modelReasoningLevels").value.split(/[,，]/).map(v=>v.trim()).filter(Boolean)};
  m.parameter_support={temperature:$("modelParamTemperature").checked,top_p:$("modelParamTopP").checked,frequency_penalty:$("modelParamFrequency").checked,max_tokens:$("modelParamMaxTokens").checked};
  const optionalNumber=id=>$(id).value.trim()===""?null:Number($(id).value);
  m.defaults={...(m.defaults||{}),reasoning_level:$("modelDefaultReasoning").value,web_search:$("modelDefaultSearch").value,temperature:optionalNumber("modelDefaultTemperature"),top_p:optionalNumber("modelDefaultTopP"),frequency_penalty:optionalNumber("modelDefaultFrequency")};
}
function routeSource(){ return state.modelRouteScope==="book"?state.bookModelRoutes:((state.modelConfig||{}).routes||{}); }
function renderModelRoutes(){
  const models=modelProfiles(), source=routeSource(), isBook=state.modelRouteScope==="book";
  $("modelRouteRows").innerHTML=MODEL_STAGES.map(([stage,label])=>{ const r=source[stage]||{}, o=r.overrides||{}; const options=models.map(m=>`<option value="${escapeHtml(m.model_id)}" ${m.model_id===r.primary_model_id?"selected":""}>${escapeHtml(m.name)}</option>`).join(""); const fallback=(r.fallback_model_ids||[]).map(id=>((state.modelConfig.models||{})[id]||{}).name||id).join(", "); return `<div class="route-card" data-stage="${stage}"><h5>${label}</h5><label class="route-inherit"><input data-field="inherit" type="checkbox" ${isBook&&r.inherit?"checked":""} ${isBook?"":"disabled"}>继承用户默认</label><label><span>主模型</span><select data-field="primary">${options}</select></label><label><span>备用链（按顺序逗号分隔）</span><input data-field="fallback" value="${escapeHtml(fallback)}"></label><label><span>推理等级</span><select data-field="reasoning"><option value="">继承档案</option>${["off","low","medium","high","xhigh","max"].map(v=>`<option ${o.reasoning_level===v?"selected":""}>${v}</option>`).join("")}</select></label><label><span>联网策略</span><select data-field="search"><option value="">继承档案</option><option value="off" ${o.web_search==="off"?"selected":""}>关闭</option><option value="on_demand" ${o.web_search==="on_demand"?"selected":""}>按需</option><option value="always" ${o.web_search==="always"?"selected":""}>总是</option></select></label><label><span>上下文上限（0 继承）</span><input data-field="context" type="number" min="0" value="${o.context_window||0}"></label><label><span>输出上限（0 继承）</span><input data-field="output" type="number" min="0" value="${o.max_output_tokens||0}"></label></div>`; }).join("");
}
function collectRenderedRoutes(){
  const byName={}; for(const m of modelProfiles()){ byName[m.name]=m.model_id; byName[m.model]=m.model_id; byName[m.model_id]=m.model_id; }
  const routes={}; document.querySelectorAll("#modelRouteRows .route-card").forEach(card=>{ const get=name=>card.querySelector(`[data-field="${name}"]`); const primary=get("primary").value; const fallback=get("fallback").value.split(/[,，]/).map(v=>byName[v.trim()]).filter((v,i,a)=>v&&v!==primary&&a.indexOf(v)===i); routes[card.dataset.stage]={primary_model_id:primary,fallback_model_ids:fallback,inherit:!!get("inherit").checked,overrides:{reasoning_level:get("reasoning").value||null,web_search:get("search").value||null,context_window:Number(get("context").value||0)||null,max_output_tokens:Number(get("output").value||0)||null}}; }); return routes;
}
function renderModelCenter(){ setModelCenterSelects(); fillProviderEditor(); fillModelEditor(); $("modelRouteScope").value=state.modelRouteScope; renderModelRoutes(); }
function addProviderTemplate(template){
  storeProviderEditor(); storeModelEditor(); const providerId=newProfileId("provider"), modelId=newProfileId("model");
  const values=template==="ollama"?["本地 Ollama","ollama_native","http://127.0.0.1:11434","qwen3"]:template==="responses"?["OpenAI Responses","openai_responses","https://api.openai.com/v1","gpt-5"]:["DeepSeek","openai_chat","https://api.deepseek.com","deepseek-v4-flash"];
  state.modelConfig.providers[providerId]={provider_id:providerId,name:values[0],protocol:values[1],base_url:values[2],api_key:"",auth_header:"Authorization",auth_prefix:"Bearer ",headers:{},timeout_seconds:180,max_retries:1,enabled:true,allow_insecure_http:template==="ollama"};
  state.modelConfig.models[modelId]={model_id:modelId,provider_id:providerId,name:values[3],model:values[3],context_window:32768,max_output_tokens:32768,template:template==="ollama"?"ollama":template==="responses"?"openai_responses":"deepseek",capabilities:{streaming:true,tools:true,native_web_search:template==="responses",reasoning:true,structured_output:true,supported_reasoning_levels:template==="responses"?["off","low","medium","high","xhigh","max"]:["off","high","max"]},defaults:{temperature:.7,top_p:.9,frequency_penalty:0,max_output_tokens:16384,reasoning_level:"off",web_search:"off"},parameter_support:{temperature:true,top_p:true,frequency_penalty:template==="responses",max_tokens:true},extra_body:{},enabled:true};
  for(const [stage] of MODEL_STAGES){ const r=state.modelConfig.routes[stage]||{}; if(!r.primary_model_id) state.modelConfig.routes[stage]={primary_model_id:modelId,fallback_model_ids:[],overrides:{}}; }
  state.selectedProviderId=providerId; state.selectedModelProfileId=modelId; renderModelCenter();
}
function deleteProviderProfile(){ const p=state.modelConfig.providers[state.selectedProviderId]; if(!p||!confirm(`删除服务「${p.name}」及其全部模型档案？`)) return; const ids=modelProfiles().filter(m=>m.provider_id===p.provider_id).map(m=>m.model_id); delete state.modelConfig.providers[p.provider_id]; ids.forEach(id=>delete state.modelConfig.models[id]); state.selectedProviderId=""; state.selectedModelProfileId=""; renderModelCenter(); }
function addModelProfile(){ storeModelEditor(); const providerId=state.selectedProviderId||providerProfiles()[0]?.provider_id; if(!providerId) throw new Error("请先新建服务连接"); const id=newProfileId("model"); state.modelConfig.models[id]={model_id:id,provider_id:providerId,name:"新模型",model:"model-name",context_window:32768,max_output_tokens:32768,template:"generic",capabilities:{streaming:true,tools:true,native_web_search:false,reasoning:false,structured_output:true,supported_reasoning_levels:["off"]},defaults:{temperature:.7,top_p:.9,frequency_penalty:0,max_output_tokens:16384,reasoning_level:"off",web_search:"off"},parameter_support:{temperature:true,top_p:true,frequency_penalty:true,max_tokens:true},extra_body:{},enabled:true}; state.selectedModelProfileId=id; renderModelCenter(); }
function deleteModelProfile(){ const m=state.modelConfig.models[state.selectedModelProfileId]; if(!m||!confirm(`删除模型档案「${m.name}」？`)) return; delete state.modelConfig.models[m.model_id]; state.selectedModelProfileId=""; renderModelCenter(); }
async function changeModelRouteScope(){ const old=state.modelRouteScope; if(old==="book") state.bookModelRoutes=collectRenderedRoutes(); else state.modelConfig.routes=collectRenderedRoutes(); state.modelRouteScope=$("modelRouteScope").value; if(state.modelRouteScope==="book"){ requireBook(); const data=await api(`/api/books/${enc(state.currentBook)}/model-routes`); state.bookModelRoutes=data.routes||{}; } renderModelRoutes(); }
async function saveModelCenter(showToast=true){ storeProviderEditor(); storeModelEditor(); if(state.modelRouteScope==="book") state.bookModelRoutes=collectRenderedRoutes(); else state.modelConfig.routes=collectRenderedRoutes(); const data=await api("/api/model-center",{method:"PUT",headers:{"X-Sensitive-Ticket":state.sensitiveTicket},body:JSON.stringify({config:state.modelConfig})}); state.modelConfig=data.config||state.modelConfig; if(state.modelRouteScope==="book"&&state.currentBook) await api(`/api/books/${enc(state.currentBook)}/model-routes`,{method:"PUT",body:JSON.stringify({routes:state.bookModelRoutes})}); renderModelCenter(); if(showToast) toast("模型中心已保存"); }
async function testSelectedModel(){ if(!confirm("将实际运行普通、流式、推理、工具和已声明的原生搜索测试，可能产生费用与外部数据传输。继续？")) return; await saveModelCenter(false); const data=await api(`/api/model-center/models/${enc(state.selectedModelProfileId)}/test`,{method:"POST",body:"{}"}); const checks=Object.entries(data.checks||{}).map(([name,row])=>`${name}: ${row.ok?"通过":"失败"}`).join("\n"); alert(`模型：${data.model||""}\n${checks}`); }
async function discoverSelectedModels(){ await saveModelCenter(false); const data=await api(`/api/model-center/models/${enc(state.selectedModelProfileId)}/discover`,{method:"POST",body:"{}"}); alert((data.models||[]).join("\n")||"服务未返回模型"); }
async function loadSettings(){ const [base,agent,presetData,modelCenter]=await Promise.all([api("/api/settings"),api("/api/settings/agent-embedding"),api("/api/settings/presets"),api("/api/model-center")]); const text=(base.api||{}).text||{}; const image=(base.api||{}).image||{}; const baseSettings=base.settings||{}; $("baseUrl").value=text.base_url||""; $("apiModel").value=text.model||""; $("currentModel").value=text.model||baseSettings.last_model||""; $("modelRoutingMode").value=baseSettings.model_routing_mode||"standard"; renderModelOptions(baseSettings,text); $("imageBaseUrl").value=image.base_url||""; $("imageModel").value=image.model||""; $("globalUserPrompt").value=baseSettings.global_user_prompt||""; const s=(agent.settings||{}); $("agentGenerationMode").value=s.novel_generation_mode||"classic"; $("agentSkillsEnabled").checked=s.agent_skills_enabled!==false; $("agentMultiPlanEnabled").checked=!!s.agent_multi_plan_enabled; $("agentRuntimeBackend").value=s.agent_runtime_backend||"legacy"; $("retrievalBackend").value=s.retrieval_backend||"classic"; $("retrievalDefaultLimit").value=s.retrieval_default_limit??8; $("retrievalMinScore").value=s.retrieval_min_score??0; $("retrievalKeywordWeight").value=s.retrieval_keyword_weight??55; $("retrievalSemanticWeight").value=s.retrieval_semantic_weight??45; $("frameworkAutoFallback").checked=s.framework_auto_fallback!==false; $("embeddingBaseUrl").value=s.embedding_base_url||""; $("embeddingModel").value=s.embedding_model||""; $("embeddingBatchSize").value=s.embedding_batch_size??8; $("embeddingTimeoutSeconds").value=s.embedding_timeout_seconds??20; $("embeddingMaxRetries").value=s.embedding_max_retries??1; $("agentWebEnabled").checked=!!s.agent_web_enabled; $("agentWebEndpoint").value=s.agent_web_endpoint||""; $("agentWebMethod").value=s.agent_web_method||"POST"; $("agentWebTimeoutSeconds").value=s.agent_web_timeout_seconds||15; $("agentWebAuthHeader").value=s.agent_web_auth_header||"Authorization"; $("agentWebAuthPrefix").value=s.agent_web_auth_prefix??"Bearer"; $("agentWebQueryField").value=s.agent_web_query_field||"query"; $("agentWebResultsPath").value=s.agent_web_results_path||"results"; $("agentWebTitleField").value=s.agent_web_title_field||"title"; $("agentWebUrlField").value=s.agent_web_url_field||"url"; $("agentWebSnippetField").value=s.agent_web_snippet_field||"content"; $("agentWebMaxResults").value=s.agent_web_max_results||5; state.presets=presetData.presets||{}; state.defaultPresetNames=presetData.default_names||[]; $("themeSelect").value=presetData.theme||"dark"; renderPresetSelect(presetData.current_preset||Object.keys(state.presets)[0]||""); state.modelConfig=modelCenter.config||{schema_version:2,providers:{},models:{},routes:{},image:{}}; renderModelCenter(); }
...
async function saveModelRouting(){ const data=await api("/api/settings",{method:"PUT",body:JSON.stringify({settings:{model_routing_mode:$("modelRoutingMode").value}})}); $("modelRoutingMode").value=(data.settings||{}).model_routing_mode||"standard"; toast($("modelRoutingMode").value==="pro_body_flash_aux"?"已启用：正文 Pro，辅助任务 Flash":"已启用：全部任务使用当前模型"); }
...
async function loadTokens(){ const data=await api(`/api/token-log${tokenFilterQuery()}`); renderTokenFilterOptions(data.facets||{}); const summary=data.summary||{}; const sum=summary.totals||{}; const activity=summary.activity||{}; const cost=summary.estimated_cost||{}; $("tokenSummary").innerHTML=[["总输入",sum.prompt_tokens||0],["总输出",sum.completion_tokens||0],["总计",sum.total_tokens||0],["总耗时",formatDurationMs(activity.duration_ms||0)],["字符",activity.char_count||0],["汉字",activity.hanzi_count||0],["估算费用",formatEstimatedCost(cost)],["筛选记录",data.total||0],["全部记录",data.overall_total??data.total??0]].map(([k,v])=>`<div class="metric-card"><span>${k}</span><b>${v}</b></div>`).join(""); const groupBox=$("tokenGroupSummary"); if(groupBox) groupBox.innerHTML=tokenGroupCards("按日期",summary.by_date)+tokenGroupCards("按模型",summary.by_model)+tokenGroupCards("按任务类型",summary.by_operation); const rows=data.entries||[]; state.tokenEntries=rows; $("tokenTable").innerHTML=`<table><thead><tr><th>时间</th><th>操作</th><th>阶段</th><th>服务 / 协议</th><th>模型</th><th>推理</th><th>输入</th><th>输出</th><th>总计</th><th>耗时</th><th>预览</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${escapeHtml(r.timestamp)}</td><td>${escapeHtml(r.operation)}</td><td>${escapeHtml(r.stage||"旧日志")}</td><td>${escapeHtml([r.provider_name,r.protocol].filter(Boolean).join(" / ")||"-")}</td><td>${escapeHtml(r.model)}</td><td>${escapeHtml(r.reasoning_level||"-")}</td><td>${formatTokenCount(r.prompt_tokens)}</td><td>${formatTokenCount(r.completion_tokens)}</td><td>${formatTokenCount(r.total_tokens)}</td><td>${formatDurationMs(r.duration_ms)}</td><td class="token-preview" data-index="${i}" title="双击查看全文">${escapeHtml(r.content_preview||"")}</td></tr>`).join("")}</tbody></table>`; $("tokenTable").querySelectorAll(".token-preview").forEach(cell=>{ cell.ondblclick=()=>openTokenContent(Number(cell.dataset.index)); }); }
...
function openTokenContent(index){ const row=(state.tokenEntries||[])[index]; const dialog=$("tokenContentDialog"); if(!dialog||!row) return; const full=(row.content_full||"").trim(); $("tokenContentFull").textContent=full||(row.content_preview||"(无内容)"); $("tokenReasoningFull").textContent=(row.reasoning_content_full||row.reasoning_content_preview||"(无推理内容)"); $("tokenRoutingDetail").textContent=JSON.stringify({provider_id:row.provider_id,provider_name:row.provider_name,protocol:row.protocol,model_profile_id:row.model_profile_id,stage:row.stage,reasoning_level:row.reasoning_level,context_budget:row.context_budget,search_source:row.search_source,fallback_attempts:row.fallback_attempts,final_failure_reason:row.final_failure_reason},null,2); $("tokenContentMeta").textContent=[row.timestamp,row.direction==="send"?"发送":"接收",row.operation,row.stage,row.provider_name,row.protocol,row.model].filter(Boolean).join(" · "); $("tokenContentNote").classList.toggle("hidden",Boolean(full)); if(!dialog.open) dialog.showModal(); }
function closeTokenContentDialog(){ const dialog=$("tokenContentDialog"); if(dialog&&dialog.open) dialog.close(); }
...
async function cancelActiveTask(){ if(!state.activeTaskId) return; await cancelTask(state.activeTaskId); }
...
function syncActiveTaskControl(){ const button=$("cancelActiveTaskBtn"); if(button) button.classList.toggle("hidden",!state.activeTaskId); }
function connectTask(id, onPayload){ if(state.eventSource) state.eventSource.close(); state.activeTaskId=id; syncActiveTaskControl(); state.streamBuffer=""; $("streamText").textContent=""; state.eventSource=new EventSource(`/api/tasks/${enc(id)}/events?token=${enc(state.token)}`); const h=e=>{ const payload=JSON.parse(e.data); updateTask(payload); if(onPayload) onPayload(payload); }; for(const t of ["started","progress","completed","failed","cancelled","finished"]) state.eventSource.addEventListener(t,h); }
...
bind("agentRegenerateBtn", regenerateCurrentAgentPlan);
bind("reviseAgentPlanBtn", reviseAgentPlan);
bind("confirmAgentPlanBtn", confirmAgentPlanGeneration);
bind("cancelAgentPlanBtn", cancelAgentPlan);
bind("closeAgentPlanDialogBtn", cancelAgentPlan);
$("agentPlanDialog")?.addEventListener("cancel", event=>{ event.preventDefault(); cancelAgentPlan().catch(error=>toast(error.message)); });
...
bind("saveModelRoutingBtn", saveModelRouting);
bind("saveApiBtn", saveApi);
bind("testApiBtn", ()=>api("/api/settings/test-connection",{method:"POST",body:"{}"}).then(()=>toast("连接成功")));
bind("saveModelCenterBtn", saveModelCenter);
bind("addDeepSeekProviderBtn", ()=>addProviderTemplate("deepseek"));
bind("addResponsesProviderBtn", ()=>addProviderTemplate("responses"));
bind("addOllamaProviderBtn", ()=>addProviderTemplate("ollama"));
bind("deleteProviderBtn", deleteProviderProfile);
bind("addModelProfileBtn", addModelProfile);
bind("deleteModelProfileBtn", deleteModelProfile);
bind("testModelProfileBtn", testSelectedModel);
bind("discoverModelsBtn", discoverSelectedModels);
bind("modelRouteScope", changeModelRouteScope, "change");
bind("providerProfileSelect", ()=>{ storeProviderEditor(); state.selectedProviderId=$("providerProfileSelect").value; fillProviderEditor(); }, "change");
bind("modelProfileSelect", ()=>{ storeModelEditor(); state.selectedModelProfileId=$("modelProfileSelect").value; fillModelEditor(); }, "change");
...
bind("closeTokenContentDialogBtn", closeTokenContentDialog); bind("closeTokenContentDialogFooterBtn", closeTokenContentDialog);
bind("cancelActiveTaskBtn", cancelActiveTask);
