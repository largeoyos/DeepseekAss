...
def extract_world_bible_from_segments(
    ...
    *,
    ...
    """Delegate segment extraction and all merge semantics to core.world_bible."""
    from core.world_bible import WorldBible, _world_bible_to_extracted_data, extract_and_merge_world_bible

    bible = WorldBible()
    errors = []
    for index, (title, content) in enumerate(segments, 1):
        ...
        try:
            bible = extract_and_merge_world_bible(
                ...
                chapter_version=1,
                global_user_prompt=global_user_prompt,
                story_context=f"导入段落：{title}",
                xp_mode=xp_mode,
            )
        except Exception as exc:
            errors.append(f"段落 {index}「{title}」提取失败: {exc}")
    data = _world_bible_to_extracted_data(bible)
    data["_errors"] = errors
    data["_diagnostics"] = dict(getattr(bible, "diagnostics", {}) or {})
    return data
