...
{{
  "characters": [
    {{
      ...
      "current_age": "本段结尾时的当前年龄，可写约数（正文无依据则空字符串）",
      "age_basis": "年龄判断依据，如出生日期+当前日期或原文明示（无依据则空字符串）",
      "life_stage": "本段结尾时的人生/身份阶段，如童年、大学一年级、孕中期（无依据则空字符串）",
  ...
  "story_clock": {{
    "current_date": "本段结尾的故事内日期/纪年；不明确则空字符串",
    "time_of_day": "本段结尾时段；不明确则空字符串",
    "elapsed_time": "累计流逝时间或本段明确推进的时间；不明确则空字符串",
    "story_phase": "当前阶段，如入学第一周、战争第三年；不明确则空字符串",
...
{{
  ...
  "author_plan": "【作者规划】把主线目标、阶段目标、人物弧光、本卷主题、节奏要求、禁写事项分条整理。注意这是未来规划，不要写成已经发生的事实。"
}}
...
def _merge_foreshadowing(target: list[dict], item: dict, chapter_marker: int) -> None:
    ...
    payload = {
        ...
        "introduced_chapter": int(item.get("introduced_chapter", 0) or chapter_marker),
        "last_touched_chapter": int(item.get("last_touched_chapter", 0) or chapter_marker),
...
{{
  ...
  "locations": [...],
  "rules": ["去重后的规则列表"],
  "timeline": [{"event": "事件", "significance": "意义", "occurrence_count": 1, "key_passages": [...], "foreshadowing_hints": [...]}],
  "plot_threads": [
    {{
      "name": "剧情线名",
      "status": "active/resolved/dormant",
  ...
  "global_foreshadowing": [
    {{"hint": "伏笔", "relates_to": "关联", "status": "open/noticed/advanced/resolved/dormant", "next_step": "推进方式", "reveal_rule": "回收限制"}}
  ]
}}
    ...
    for idx, (title, content) in enumerate(segments):
        ...
        if isinstance(incoming_clock, dict):
            for field_name in ("current_date", "time_of_day", "elapsed_time", "story_phase", "calendar_system"):
                ...
                if value:
                    merged["story_clock"][field_name] = value[:120]
            if merged["story_clock"]:
                merged["story_clock"]["source_chapter"] = chapter_marker
        ...
        for ch in data.get("characters", []):
            ...
            if existing:
                ...
                merged["characters"].append({
                    ...
                    "arc": ch.get("arc", "")[:200],
                    "birth_date": ch.get("birth_date", "")[:60],
                    "current_age": ch.get("current_age", "")[:60],
                    "age_basis": ch.get("age_basis", "")[:120],
                    "life_stage": ch.get("life_stage", "")[:100],
        ...
        for t in data.get("timeline", []):
            ...
            if event:
                try:
                    incoming_count = max(1, int(t.get("occurrence_count", 1) or 1))
                except (TypeError, ValueError):
                    incoming_count = 1
                existing = next(
                    (item for item in merged["timeline"] if _norm_key(item.get("event", "")) == _norm_key(event)),
                    None,
                )
                passages = [_verify_verbatim(kp, content) for kp in t.get("key_passages", [])]
                hints = [fh[:50] for fh in t.get("foreshadowing_hints", [])]
                if existing:
                    existing["occurrence_count"] = max(1, int(existing.get("occurrence_count", 1) or 1)) + incoming_count
                    existing["significance"] = _append_text_unique(
                        existing.get("significance", ""),
                        t.get("significance", "")[:200],
                        500,
                    )
                    _merge_list_dedup(existing.setdefault("key_passages", []), passages)
                    _merge_list_dedup(existing.setdefault("foreshadowing_hints", []), hints)
                else:
        ...
        for item in data.get("global_foreshadowing", []):
            _merge_foreshadowing(merged["global_foreshadowing"], item, chapter_marker)
...
def _run_synthesis(client, merged: dict, model: str, global_user_prompt: str = "") -> None:
    ...
    accumulated = _json.dumps({
        "story_clock": merged.get("story_clock", {}),
    ...
    syn_clock = syn_data.get("story_clock", {})
    if isinstance(syn_clock, dict):
        for field_name in ("current_date", "time_of_day", "elapsed_time", "story_phase", "calendar_system"):
            value = str(syn_clock.get(field_name, "")).strip()
            if value:
                merged.setdefault("story_clock", {})[field_name] = value[:120]
    ...
    for item in syn_data.get("global_foreshadowing", []):
        _merge_foreshadowing(merged.setdefault("global_foreshadowing", []), item, 0)
...
def generate_novel_settings_from_world_bible(
    ...
    try:
        ...
        return {
            ...
            "author_plan": data.get("author_plan", ""),
        }
    except Exception:
        return {
            "background_story": "",
